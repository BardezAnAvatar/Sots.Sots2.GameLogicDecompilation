...
namespace Kerberos.Sots.UI
{
	...
	{
		public OverlayPiracyMission(App game, StarMapState state, StarMap starmap, string template = "OverlaySurveyMission") : base(game, state, starmap, MissionType.PIRACY, template)
		{
		}
		...
		{
            Kerberos.Sots.StarFleet.StarFleet.SetPiracyMission(this.App.Game, base.SelectedFleet, base.TargetSystem, this._useDirectRoute, 0, 0, base.GetDesignsToBuild());
			...
			if (admiralInfo != null)
			{
				string cueName = string.Format("STRAT_006-01_{0}_{1}StrikeMissionConfirmation", this.App.GameDatabase.GetFactionName(this.App.GameDatabase.GetPlayerFactionID(this.App.LocalPlayer.ID)), admiralInfo.GetAdmiralSoundCueContext(this.App.AssetDatabase));
		...
		protected override string GetMissionDetailsTitle()
		{
			...
			return string.Format(App.Localize("@UI_PIRACY_OVERLAY_MISSION_TITLE"), starSystemInfo.Name.ToUpperInvariant());
		}
		protected override void OnRefreshMissionDetails(MissionEstimate estimate)
		{
			...
			base.AddMissionTime(2, App.Localize("@UI_PIRACY_TIME"), estimate.TurnsAtTarget, hint);

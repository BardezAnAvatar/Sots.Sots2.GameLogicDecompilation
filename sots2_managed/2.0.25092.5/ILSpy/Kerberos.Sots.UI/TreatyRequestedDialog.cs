...
namespace Kerberos.Sots.UI
{
	internal class TreatyRequestedDialog : Dialog
	{
		...
		public TreatyRequestedDialog(App game, int treatyId) : base(game, "dialogTreatyRequest")
		{
		...
		public override void Initialize()
		{
			...
			{
				this._app.UI.SetVisible(current.Value, current.Key == this._treatyInfo.Type);
			}
			if (this._treatyInfo.Type == TreatyType.Limitation)
			{
				this._app.UI.SetPropertyInt(base.UI.Path(new string[]
				{
					base.ID,
					"pnlDialogPanel"
				}), "height", 280);
			...
			case TreatyType.Limitation:
			{
				...
				{
					this._app.UI.AddItem("lstConsequence", string.Empty, current3.ID, string.Format("{0} {1}", App.Localize(TreatyEditDialog.ConsequenceTypeLocMap[current3.Type]), current3.ConsequenceValue));
				}

...
namespace Kerberos.Sots.UI
{
	internal class DebugTechDialog : Dialog
	{
		...
		public DebugTechDialog(GameSession game, int intel_mission_id) : base(game.App, "counterIntel_Standard")
		{
		...
		protected void SetSyncedTechFamily(TechFamily family)
		{
			...
			foreach (Tech tech in 
				from x in this._game.AssetDatabase.MasterTechTree.Technologies
				where x.Family == family.Id
				select x)
			{
				...
				this._game.UI.SetChecked(base.UI.Path(new string[]
				{
					itemGlobalID,
					"techtoggle"
				}), this._CheckedTechs.Any((KeyValuePair<string, bool> x) => x.Key == tech.Id) && this._CheckedTechs[tech.Id]);
				...
				this._game.UI.SetPropertyString(base.UI.Path(new string[]
				{
					itemGlobalID,
					"techtoggle"
				}), "id", "techtoggle|" + this._FamilyID.FirstOrDefault((KeyValuePair<int, TechFamily> x) => x.Value == family).Key.ToString() + "|" + tech.Id);
				UICommChannel arg_2F0_0 = this._game.UI;
				UICommChannel arg_2EB_0 = base.UI;
				string[] array = new string[2];
				array[0] = itemGlobalID;
				array[1] = "techtoggle|" + this._FamilyID.FirstOrDefault((KeyValuePair<int, TechFamily> x) => x.Value == family).Key.ToString() + "|" + tech.Id;
				string globalID = arg_2F0_0.GetGlobalID(arg_2EB_0.Path(array));
				this._TechToggles.Add(tech.Id, globalID);
				this._game.UI.SetText(this._game.UI.Path(new string[]
				{
					itemGlobalID,
					"techLabel"
				}), this._game.AssetDatabase.MasterTechTree.Technologies.FirstOrDefault((Tech x) => x.Id == tech.Id).Name);
				this._game.UI.SetPropertyString(this._game.UI.Path(new string[]
				{
					itemGlobalID,
					"icon"
				}), "sprite", Path.GetFileNameWithoutExtension(this._game.AssetDatabase.MasterTechTree.Technologies.FirstOrDefault((Tech x) => x.Id == tech.Id).Icon));
			}
		}

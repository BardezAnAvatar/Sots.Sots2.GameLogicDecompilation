...
namespace Kerberos.Sots.UI
{
	internal class SpecialProjectDialog : Dialog
	{
		...
		public SpecialProjectDialog(App game, string template = "dialogSpecialProjects") : base(game, template)
		{
		}
		public override void Initialize()
		{
			...
			}), "text", App.Localize("@UI_RESEARCH_CONTEXT_SPECIAL"));
		...
		private void RefreshProjects()
		{
			...
			this._specialPoints = (int)((float)this._researchPoints * playerInfo.RateResearchSpecialProject);
		...
		private void RefreshSliders()
		{
			foreach (SpecialProjectInfo current in this._rates)
			{
				...
				int num2 = (this._specialPoints > 0) ? ((int)((float)num / (current.Rate * (float)this._specialPoints))) : 10000;
				if (current.Progress == -1)
				{
					num2 = ((this._specialPoints > 0) ? (num / this._specialPoints) : 10000);
				}
				num2++;
				if (num2 < 0)
				{
		...
		private void UpdateResearchSliders(PlayerInfo playerInfo, string iChanged)
		{
			...
			{
				this._app.UI.SetSliderValue(SpecialProjectDialog.UIGovernmentResearchSlider, (int)num);
			}
		...
		protected override void OnPanelMessage(string panelName, string msgType, string[] msgParams)
		{
			if (msgType == "button_clicked")
			{
				...
				if (panelName.Contains("startButton"))
				{
					...
					if (specialProjectInfo != null && specialProjectInfo.Progress == -1)
					{
						...
						this._confirmProjectChangeDialog = this._app.UI.CreateDialog(new GenericQuestionDialog(this._app, App.Localize("@UI_SPECIAL_RESEARCH_START_TITLE"), App.Localize("@UI_SPECIAL_RESEARCH_START_DESC"), "dialogGenericQuestion"), null);
						return;
					}
				}
				else
				{
					if (panelName.Contains("cancelButton"))
					{
						...
						if (specialProjectInfo2 != null && specialProjectInfo2.Progress > -1)
						{
							...
							this._confirmProjectChangeDialog = this._app.UI.CreateDialog(new GenericQuestionDialog(this._app, App.Localize("@UI_SPECIAL_RESEARCH_CANCEL_TITLE"), App.Localize("@UI_SPECIAL_RESEARCH_CANCEL_DESC"), "dialogGenericQuestion"), null);
							return;
						}
					}
				}
			...
			else
			{
				if (msgType == "slider_value")
				{
					...
					{
						if (panelName == SpecialProjectDialog.UICurrentProjectSlider)
						{
						...
						{
							if (panelName == SpecialProjectDialog.UISpecialProjectSlider)
							{
							...
							{
								if (panelName == SpecialProjectDialog.UISalvageResearchSlider)
								{

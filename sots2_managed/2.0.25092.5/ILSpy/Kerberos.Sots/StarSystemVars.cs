...
namespace Kerberos.Sots
{
	public class StarSystemVars
	{
		...
		private float SizeStep
		{
			get
			{
				return (this.PlanetMaxRadius - this.PlanetMinRadius) / 10f;
			}
		}
		...
		{
			if (radius >= this.PlanetMinRadius)
			{
				float num = (radius - this.PlanetMinRadius) / this.SizeStep;
		...
		public static void LoadXml(string fileName)
		{
			...
			XmlElement xmlElement = xmlDocument["StarSystemVars"];

...
namespace Kerberos.Sots.Strategy
{
	internal class TurnEvent
	{
		...
		public bool IsCombatEvent
		{
			get
			{
				return this.EventType == TurnEventType.EV_COMBAT_WIN || this.EventType == TurnEventType.EV_COMBAT_LOSS || this.EventType == TurnEventType.EV_COMBAT_DRAW;
			}
		}
		...
		{
			TurnEventMacros injectionObject = new TurnEventMacros(db.AssetDatabase, db, this);
			this.EventDesc = App.Localize("@" + this.EventMessage.ToString()).Inject(injectionObject);
		}
		public void RebuildEventName(GameDatabase db)
		{
			TurnEventMacros injectionObject = new TurnEventMacros(db.AssetDatabase, db, this);
			this.EventName = App.Localize("@" + this.EventType.ToString()).Inject(injectionObject);
		}
		...
		{
			string factionName = game.GameDatabase.GetFactionName(game.GameDatabase.GetPlayerFactionID(e.PlayerID));
			...
			case TurnEventMessage.EM_SHIPS_BUILT_SINGLE:
				return string.Format("ui\\events\\single_{0}_shipbuilt.tga", factionName);
			case TurnEventMessage.EM_SHIPS_BUILT_MULTIPLE:
				return string.Format("ui\\events\\multi_{0}_shipsbuilt.tga", factionName);
			...
			case TurnEventMessage.EM_COLONY_ESTABLISHED:
				return string.Format("ui\\events\\colony_estab_{0}.tga", factionName);
			case TurnEventMessage.EM_COLONY_SUPPORT:
				if (!(factionName == "loa"))
				{
					return string.Format("ui\\events\\colony_supplied_{0}.tga", factionName);
				}
				return "ui\\events\\biosphere_burned.tga";
			case TurnEventMessage.EM_COLONY_SUPPORT_COMPLETE:
				if (!(factionName == "loa"))
				{
					return string.Format("ui\\events\\colony_sup_{0}.tga", factionName);
				}
				return "ui\\events\\biosphere_finished_burning.tga";
			case TurnEventMessage.EM_COLONY_SELF_SUFFICIENT:
				return string.Format("ui\\events\\colony_selfsuf_{0}.tga", factionName);
			case TurnEventMessage.EM_COLONY_ABANDONED:
				return string.Format("ui\\events\\colony_abandoned_{0}.tga", factionName);
			...
			case TurnEventMessage.EM_SUULKA_ARRIVES:
				return string.Format("ui\\events\\{0}_arrives.tga", (game.GameDatabase.GetShipInfo(e.ShipID, true) != null) ? game.GameDatabase.GetShipInfo(e.ShipID, true).DesignInfo.Name.ToLower().Replace(" ", "") : "?");
			case TurnEventMessage.EM_SUULKA_LEAVES:
				return string.Format("ui\\events\\{0}_leaves.tga", (game.GameDatabase.GetShipInfo(e.ShipID, true) != null) ? game.GameDatabase.GetShipInfo(e.ShipID, true).DesignInfo.Name.ToLower().Replace(" ", "") : "?");
			case TurnEventMessage.EM_ADMIRAL_PROMOTED:
				return string.Format("ui\\events\\promote_admiral_{0}.tga", factionName);
			...
			case TurnEventMessage.EM_ADMIRAL_CAPTURED:
				return string.Format("ui\\events\\{0}_admiral_captured.tga", factionName);
			case TurnEventMessage.EM_ADMIRAL_ESCAPES:
				return string.Format("ui\\events\\{0}_admiral_escapes.tga", factionName);
			case TurnEventMessage.EM_ADMIRAL_DEFECTS:
				return string.Format("ui\\events\\{0}_admiral_joins_rebels.tga", factionName);
			...
			case TurnEventMessage.EM_SUULKA_DIES:
				return string.Format("ui\\events\\{0}_dies.tga", (game.GameDatabase.GetDesignInfo(e.DesignID) != null) ? game.GameDatabase.GetDesignInfo(e.DesignID).Name.ToLower().Replace(" ", "") : "?");
			...
			case TurnEventMessage.EM_TRADE_STIMULUS:
				return string.Format("ui\\events\\{0}_freighter_built.tga", factionName);
			case TurnEventMessage.EM_FREIGHTERS_BUILT:
				return string.Format("ui\\events\\{0}_multi_freighters_built.tga", factionName);
			...
			case TurnEventMessage.EM_SURVEY_INDEPENDENT_RACE_FOUND:
				return string.Format("ui\\events\\encounter_{0}.tga", factionName2);
		...
		{
			List<TurnEventMessage> list = new List<TurnEventMessage>();
			List<TurnEventMessage> list2 = new List<TurnEventMessage>();
			foreach (TurnEventMessage turnEventMessage in Enum.GetValues(typeof(TurnEventMessage)))
			{
				TurnEvent turnEvent = new TurnEvent();
				turnEvent.TargetPlayerID = game.LocalPlayer.ID;
				turnEvent.PlayerID = game.LocalPlayer.ID;
				turnEvent.EventMessage = turnEventMessage;
				SuulkaInfo suulkaInfo = game.GameDatabase.GetSuulkas().FirstOrDefault<SuulkaInfo>();
				if (suulkaInfo != null)
				{
					turnEvent.ShipID = suulkaInfo.ShipID;
					turnEvent.DesignID = game.GameDatabase.GetShipInfo(suulkaInfo.ShipID, false).DesignID;
				}
				string turnEventSprite = TurnEvent.GetTurnEventSprite(game, turnEvent);
				if (turnEventSprite == "ui\\events\\events_static_noise.tga")
				{
					list2.Add(turnEventMessage);
				}
				else
				{
					if (turnEventSprite == "" || !File.Exists(game.App.GameRoot + "\\" + turnEventSprite))
					{
						list.Add(turnEventMessage);
					}
				}
			}
			App.Log.Trace("==MISSING NEWS EVENT HOOKUPS==", "log");
			foreach (TurnEventMessage current in list2)
			{
				App.Log.Trace(current.ToString(), "log");
			}
			App.Log.Trace("==MISSING NEWS EVENT ART==", "log");
			foreach (TurnEventMessage current2 in list)
			{
				App.Log.Trace(current2.ToString(), "log");
			}
		}
	}
}

	...
	internal class ComparativeAnalysysState : GameState, IKeyBindListener
	{
		private class CombatShipQueue
		{
			...
			public void AddDesign(App game, ShipBuilder builder, int count, int playerID)
			{
				...
				ComparativeAnalysysState.CombatShipQueue.SetText(game, "Added ship " + CombatConfig.GetGameObjectsElement(this._config).ChildNodes.Count);
			}
		...
		protected override void OnPrepare(GameState prev, object[] stateParams)
		{
			...
			base.App.UI.LoadScreen("ComparativeAnalysys");
			...
			this.EncounteredDesigns = base.App.GameDatabase.GetDesignsEncountered(base.App.LocalPlayer.ID).ToList<DesignInfo>();
			...
			IEnumerable<Faction> arg_53C_0 = base.App.AssetDatabase.Factions;
			List<ShipSectionAsset> list = new List<ShipSectionAsset>();
			foreach (PlayerInfo player in base.App.GameDatabase.GetStandardPlayerInfos())
			{
				if (this.EncounteredDesigns.Any(delegate(DesignInfo x)
				{
					if (x.PlayerID == player.ID)
					{
						return x.DesignSections.Any((DesignSectionInfo y) => ComparativeAnalysysState.DesignScreenAllowsShipClass(y.ShipSectionAsset));
					}
					return false;
				}))
				{
					list.AddRange(
						from x in base.App.Game.GetAvailableShipSections(player.ID)
						where ComparativeAnalysysState.DesignScreenAllowsShipClass(x)
						select x);
				}
			}
		...
		private void CollectWeapons(ShipSectionAsset section)
		{
			...
			for (int j = 0; j < section.Modules.Length; j++)
			{
				...
				IEnumerable<LogicalModule> enumerable = LogicalModule.EnumerateModuleFits(base.App.GameDatabase.GetAvailableModules(base.App.AssetDatabase, this._SelectedPlayer), section, j, this._showDebugControls).ToList<LogicalModule>();
		...
		private void RepopulateDebugControls()
		{
			if (this._showDebugControls)
			{
				base.App.UI.ClearItems(ComparativeAnalysysState.DebugEmpireColorList);
				for (int i = 0; i < Player.DefaultPrimaryPlayerColors.Count; i++)
				{
					...
					{
						"AddItem",
						ComparativeAnalysysState.DebugEmpireColorList,
						i.ToString(),
						"colorbox"
					});
					base.App.UI.Send(new object[]
				...
				base.App.UI.ClearItems(ComparativeAnalysysState.DebugAnimTracks);
				for (int j = 0; j < ComparativeAnalysysState._debugAnimTracks.Length; j++)
				{
					base.App.UI.AddItem(ComparativeAnalysysState.DebugAnimTracks, string.Empty, j, ComparativeAnalysysState._debugAnimTracks[j]);
				}
				this.SetSelectedAnimTrack(ComparativeAnalysysState._debugAnimTracks[0], string.Empty);
				base.App.UI.ClearItems(ComparativeAnalysysState.DebugAnimMode);
				for (int k = 0; k < ComparativeAnalysysState._debugAnimMode.Length; k++)
				{
					base.App.UI.AddItem(ComparativeAnalysysState.DebugAnimMode, string.Empty, k, ComparativeAnalysysState._debugAnimMode[k]);
				}
				base.App.UI.SetSelection(ComparativeAnalysysState.DebugAnimMode, 0);
			}
		}
		...
		{
			IEnumerable<DesignInfo> designInfosForPlayer = base.App.GameDatabase.GetDesignInfosForPlayer(this._SelectedPlayer);
		...
		private void CollectWeapons(ShipSectionAsset section, IWeaponShipData weaponBank, WeaponEnums.WeaponSizes turretSize, WeaponEnums.TurretClasses turretClass, LogicalWeapon includeSelectedWeapon)
		{
			if (WeaponEnums.IsBattleRider(turretClass))
			{
				LogicalWeapon logicalWeapon = LogicalWeapon.EnumerateWeaponFits(section.Faction, section.SectionName, 
					from weapon in base.App.GameDatabase.GetAvailableWeapons(base.App.AssetDatabase, this._SelectedPlayer)
		...
		protected override void OnEnter()
		{
			...
			base.App.UI.SetScreen("ComparativeAnalysys");
			...
			base.App.UI.SetPropertyBool(ComparativeAnalysysState.UISectionList(ShipSectionType.Command), "only_user_events", true);
			base.App.UI.SetPropertyBool(ComparativeAnalysysState.UISectionList(ShipSectionType.Engine), "only_user_events", true);
			base.App.UI.SetPropertyBool(ComparativeAnalysysState.UISectionList(ShipSectionType.Mission), "only_user_events", true);
			...
			string factionName = base.App.GameDatabase.GetFactionName(base.App.GameDatabase.GetPlayerFactionID(this._SelectedPlayer));
		...
		public ComparativeAnalysysState(App game) : base(game)
		{
		}
		...
		private void PopulateModuleSelector(List<ModuleData> modules)
		{
			...
			this._moduleSelector.SetAvailableModules(
				from x in modules
				select x.Module, (this._selectedModule.SelectedModule != null) ? this._selectedModule.SelectedModule.Module : null, true);
		...
		private void PopulatePsionicSelector(IEnumerable<LogicalPsionic> psionics, LogicalPsionic selected)
		{
			...
			IEnumerable<LogicalPsionic> modules = 
				from x in psionics
				where this.PsiNotSelected(x) && x.IsAvailable(base.App.GameDatabase, this._SelectedPlayer, false)
				select x;
		...
		protected void SelectFilter(string panel, bool enabled)
		{
			...
			if (enabled)
			{
				if (panel == "filterAll")
				{
					using (Dictionary<string, ComparativeAnalysysState.BankFilter>.Enumerator enumerator = ComparativeAnalysysState.BankFilters.GetEnumerator())
					{
						while (enumerator.MoveNext())
						{
							KeyValuePair<string, ComparativeAnalysysState.BankFilter> current = enumerator.Current;
			...
			IL_C5:
			if (!(
				from x in ComparativeAnalysysState.BankFilters
				where x.Value.Enabled
				select x).Any<KeyValuePair<string, ComparativeAnalysysState.BankFilter>>())
			{
		...
		protected override void UICommChannel_OnPanelMessage(string panelName, string msgType, string[] msgParams)
		{
			if (msgType == "filter_mode")
			{
				...
				if (weaponBank != null)
				{
					this.SetWeaponsAllFilterMode(weaponBank.Weapon, filterMode);
					return;
				}
			}
			else
				...
				else
				{
					if (msgType == "button_clicked")
					{
						if (panelName == "expandFilters")
						{
							...
							base.App.UI.SetVisible("BankFilter", ComparativeAnalysysState.FilterVisible);
							return;
						}
					...
					else
					{
						if (msgType == "color_changed")
						{
							...
							{
								float x = float.Parse(msgParams[0]);
						...
						else
						{
							if (msgType == "list_sel_changed")
							{
								...
								if (panelName == "gamePlayerSelect")
								{
									int num = int.Parse(msgParams[0]);
									this._SelectedPlayer = num;
									string factionName = base.App.GameDatabase.GetFactionName(base.App.GameDatabase.GetPlayerFactionID(num));
		...
		private void SetSelectedAnimTrack(string trackName, string trigger)
		{
			...
			{
				base.App.UI.SetText(ComparativeAnalysysState.DebugAnimEdit, trackName);
			}
			if (trigger == ComparativeAnalysysState.DebugAnimTracks)
			{
				int num = -1;
				for (int i = 0; i < ComparativeAnalysysState._debugAnimTracks.Length; i++)
				{
					if (ComparativeAnalysysState._debugAnimTracks[i] == trackName)
					{
						num = i;
						break;
					}
				...
				if (num >= 0)
				{
					base.App.UI.SetSelection(ComparativeAnalysysState.DebugAnimTracks, num);
					return;
				}
				base.App.UI.ClearSelection(ComparativeAnalysysState.DebugAnimTracks);
			}
		}
		...
		private void SetSelectedDesign(int designId, string trigger)
		{
			...
			for (int i = 0; i < designSections.Length; i++)
			{
				...
				int num = 0;
				foreach (WeaponBankInfo current2 in section.WeaponBanks)
				{
					WeaponBankShipData currentWeaponBank = this._selection.GetCurrentWeaponBank(base.App.GameDatabase, current2);
					if (currentWeaponBank != null)
					{
						if (current2.WeaponID.HasValue)
						{
							string weaponFile = base.App.GameDatabase.GetWeaponAsset(current2.WeaponID.Value);
							currentWeaponBank.SelectedWeapon = currentWeaponBank.Weapons.FirstOrDefault((LogicalWeapon x) => x.FileName == weaponFile);
							currentWeaponBank.FiringMode = current2.FiringMode;
							currentWeaponBank.FilterMode = current2.FilterMode;
						}
						if (current2.DesignID.HasValue)
						{
							currentWeaponBank.SelectedDesign = (current2.DesignID.HasValue ? current2.DesignID.Value : 0);
						}
						num++;
					}
				}
				foreach (DesignModuleInfo current3 in section.Modules)
				{
					string moduleAsset = base.App.GameDatabase.GetModuleAsset(current3.ModuleID);
					ModuleShipData currentModuleMount = this._selection.GetCurrentModuleMount(base.App.AssetDatabase.ShipSections.First((ShipSectionAsset x) => x.FileName == section.FilePath), current3.MountNodeName);
					if (currentModuleMount == null)
					{
						ComparativeAnalysysState.Warn(string.Format("Module mount {0} not found for design '{1}', section '{2}'.", current3.MountNodeName, designInfo.Name, shipSectionAsset.FileName));
					}
					else
					{
						...
						if (current3.PsionicAbilities != null)
						{
							foreach (ModulePsionicInfo current4 in current3.PsionicAbilities)
							{
								currentModuleMount.SelectedModule.SelectedPsionic.Add(current4.Ability);
							}
						}
						if (currentModuleMount.SelectedModule != null && current3.WeaponID.HasValue)
						{
		...
		private void SetSelectedSection(ShipSectionAsset section, ShipSectionType sectionType, string trigger, bool refreshOtherSections = true, DesignInfo design = null)
		{
			...
			{
				base.App.UI.SetSelection(ComparativeAnalysysState.UISectionList(sectionType), currentSectionTypeShipData.Sections.IndexOf(sectionShipData));
			}
			if (section != null)
			{
				ShipDesignUI.SyncSectionArmor(base.App, ComparativeAnalysysState.UISectionStats(sectionType), section, design);
			}
		...
		private void SyncWeaponUi()
		{
			...
			if (!ComparativeAnalysysState.BankFilters.Any((KeyValuePair<string, ComparativeAnalysysState.BankFilter> x) => x.Key == "filterAll" && x.Value.Enabled))
			{
				IEnumerable<KeyValuePair<string, ComparativeAnalysysState.BankFilter>> source = 
					from x in ComparativeAnalysysState.BankFilters
		...
		{
			base.App.UI.ClearItems(ComparativeAnalysysState.DebugFactionsList);
			for (int i = 0; i < this._factionsList.Count; i++)
			{
				base.App.UI.AddItem(ComparativeAnalysysState.DebugFactionsList, string.Empty, i, this._factionsList[i].Name);
			}
		}
		public void SetSelectedFaction(string factionName, string trigger)
		{
			...
			if (trigger != ComparativeAnalysysState.DebugFactionsList)
			{
				...
				base.App.UI.SetSelection(ComparativeAnalysysState.DebugFactionsList, userItemId);
			}
			if (trigger != "init")
			{
				this.CurrentShipChanged();
		...
		private IEnumerable<RealShipClasses> CollectAvailableShipClasses()
		{
			...
			foreach (RealShipClasses shipClass in Enum.GetValues(typeof(RealShipClasses)).Cast<RealShipClasses>())
			{
				...
				if (this.EncounteredDesigns.Any((DesignInfo x) => this._SelectedPlayer == x.PlayerID && x.GetRealShipClass() == shipClass))
				{
		...
		{
			base.App.UI.ClearItems("gamePlayerSelect");
			foreach (PlayerInfo ply in base.App.GameDatabase.GetStandardPlayerInfos())
			{
				if (this.EncounteredDesigns.Any(delegate(DesignInfo x)
				{
				...
				{
					base.App.UI.AddItem("gamePlayerSelect", string.Empty, ply.ID, ply.Name);
				}
			}
			base.App.UI.SetSelection("gamePlayerSelect", this.EncounteredDesigns.First<DesignInfo>().PlayerID);
		}
		...
		{
			string text = ComparativeAnalysysState.UISectionList(sectionType);
			SectionTypeShipData currentSectionTypeShipData = this._selection.GetCurrentSectionTypeShipData(sectionType);
			...
			if (currentSectionTypeShipData != null)
			{
				for (int i = 0; i < currentSectionTypeShipData.Sections.Count; i++)
				{
					if (this._showDebugControls || !currentSectionTypeShipData.Sections[i].Section.IsSuulka)
					{
		...
		private DesignInfo SummarizeDesign(int playerId, bool finalsummerize = false)
		{
			...
			{
				designInfo.Name = base.App.GameDatabase.ResolveNewDesignName(playerId, string.Empty);
			}
			else
			{
				designInfo.Name = base.App.GameDatabase.ResolveNewDesignName(playerId, designInfo.Name);
			}
		...
		private void PopulateDesignList()
		{
			...
			IEnumerable<DesignInfo> source = 
				from x in this.EncounteredDesigns
				where x.GetRealShipClass() == this.SelectedClass && x.PlayerID == this._SelectedPlayer
				select x;
			List<DesignInfo> list = source.ToList<DesignInfo>();
			ShipDesignUI.PopulateDesignList(base.App, "gameDesignList", list);
			if (list.Any<DesignInfo>())
			{
				base.App.UI.SetSelection("gameDesignList", list.First<DesignInfo>().ID);
			}
		}
		...
		private void ChangeTechGroupComboSelection(ShipSectionType section, int optionIndex, string tech, bool enabled)
		{
			...
			base.App.UI.SetEnabled(this.GetTechPanel(section, optionIndex, tech), enabled);
		...
		private void SyncSectionTechs(DesignInfo design)
		{
			...
			for (int i = 0; i < designSections.Length; i++)
			{
				...
				foreach (int current in section.Techs)
				{
					...
					if (this._shipOptionGroups.Keys.Contains(shipSectionAsset.Type))
					{
		...
		private void RefreshCurrentShip()
		{
			...
			base.App.UI.SetVisible("gameCommandListBG", flag);
			base.App.UI.SetVisible("gameEngineList", flag2);
			base.App.UI.SetVisible("EngineDesign", flag2);
			base.App.UI.SetVisible("gameEngineListBG", flag2);
			...
			DesignInfo designInfo = this.SummarizeDesign(this._SelectedPlayer, false);
			IEnumerable<PlayerTechInfo> playerTechInfos = base.App.GameDatabase.GetPlayerTechInfos(this._SelectedPlayer);
			...
			if (designInfo != null)
			{
				...
				for (int i = 0; i < designSections.Length; i++)
				{
					...
					base.App.UI.SetText(ComparativeAnalysysState.UISectionList(sectionType), App.Localize(shipSectionAsset.Title));
				}
				designInfo = this.SummarizeDesign(this._SelectedPlayer, false);
			}
			if (this._targetArena != null)
			{
			...
			this._builder.New(base.App.GetPlayer(this._SelectedPlayer), (
